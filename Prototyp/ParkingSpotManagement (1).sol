...
library ParkingSpotManagement {
    ...
    function generateParkingSpots(
        ...
        for (uint256 i = 1; i <= numParkingSpots; i++) {
    ...
    function getOccupiedParkingSpots(mapping(uint256 => ParkingSpot) storage parkingSpots) internal view returns (uint256[] memory) {
        uint256 count = 0;
...
function signalRelease(
    ...
    parkingSpot.parkingFees = getCurrentParkingFees(parkingSpots, parkingSpotId, parkingFeePerMinute);
    parkingSpot.occupancyStartTime = block.timestamp; // Setzt die Startzeit der Belegung zurück
}
...
function releaseParkingSpot(
    ...
    parkingSpot.occupancyStartTime = 0;
    parkingSpot.vehicleOwner = address(0);
    parkingSpot.parkingFees = 0;
}
